...
SVC_BASELINE_URL=http://fortio-server:8077
SVC_SIDECAR_URL=http://fortio-server:8080
ECHO_RESP_SIZE=1024
PAYLOAD_SIZE=512
...
CLIENT_POD=$(oc get po | grep fortio-client | awk '{print $1}')
...
do
  ...
  CMD="fortio load -c ${i} -qps ${EXPECTED_QPS} -t ${TEST_DURATION} -a -jitter=${JITTER} -quiet -p 50,75,90,99,99.9 -r 0.001 -httpbufferkb ${HTTP_BUFFER_KB} -labels ${LABELS} -payload-size ${PAYLOAD_SIZE} ${TEST_URL}"
  oc exec ${CLIENT_POD} -c "uncaptured" -- ${CMD} 
  sleep 10
  echo -e '\n'
...
# collect metrics
oc cp ${CLIENT_POD}:/var/lib/fortio . -c "shell"
oc exec ${CLIENT_POD} -c "shell" -- find /var/lib/fortio -type f -name '*.json' -delete
